...
function login_user(){
 ...
 function send_message(){
 	...
 	if(isset($_POST['submit'])){
 		...
 		$result = mail($to, $subject, $message,$headers); //mail function doesnot work 99% of the time so experience error , email vendors block this method
		...
		$product = <<<DELIMETER
				<tr>
		            ...
		            <td>{$row['product_title']}<br>
		              <a href="index.php?edit_product&id={$row['product_id']}"><img width='200' height='100' src="../../resources/{$product_image}" alt=""></a>
		            </td>
		            <td>{$category}</td>
...
function add_product(){
	...
	if(isset($_POST['publish'])){
		...
		//use file method for imaage use file array
...
function add_category(){
	...
	if(isset($_POST['add_category'])){
		...
		if(empty($cat_title)||$cat_title==" "){
			...
			echo "<p Fira Code CodeFira Codes='bg-danger'>CATEGORY CANNOT BE EMPTY</p>";
			...
			// 	set_message("Category could not be inserted");
			// }
			// 
...
function add_user(){
	if(isset($_POST['add_user'])){
		...
		set_message("ADMIN USER CREATED");
...
function get_active_slide(){
	...
	while($row = fetch_array($query)){
		...
		<img class="slide-image" src="../resources/slider_images/{$row['slide_image']}" alt="banner 1">
		</div>
		DELIMETER;
...
function get_slides(){
	...
	while($row = fetch_array($query)){
		...
		$slides = <<<DELIMETER
		<div class="item">
		<img class="slide-image" src="../resources/slider_images/{$row['slide_image']}" alt="banner 2 and 3">
		</div>
		DELIMETER;
